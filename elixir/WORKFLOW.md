...
hooks:
  after_create: |
    git clone --depth 1 "${PROJECT_REPO}" .
    if [ -f .symphony/hooks/setup.sh ]; then
      sh .symphony/hooks/setup.sh
    fi
  before_remove: |
    if [ -f .symphony/hooks/teardown.sh ]; then
      sh .symphony/hooks/teardown.sh
    fi
agent:
